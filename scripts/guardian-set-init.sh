...
alphTokenBridgeVAA=$(npm --prefix clients/js start --silent -- generate registration -m TokenBridge -c algorand -a ${alphTokenBridge} -g ${guardiansPrivateCSV})
...
for envDest in ./solana/.env ./terra/tools/.env ./algorand/.env ./alephium/.env; do
