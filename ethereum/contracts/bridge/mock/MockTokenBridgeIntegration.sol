...
contract MockTokenBridgeIntegration {
    ...
    function completeTransferAndSwap(bytes memory encodedVm) public {
        // token bridge transfers are 131 bytes, our additional payload is 32 bytes = 163
        // len - 163 + 33 = len - 130
        bytes32 tokenAddress = encodedVm.toBytes32(encodedVm.length-130);
        // len - 163 + 65 = len - 98
        uint16 tokenChainId = encodedVm.toUint16(encodedVm.length-98);
        ...
        // additional field(s)
        bytes32 receiver = payload.toBytes32(131);
