...
import { GREEN, GRAY } from '../styles'
import { COLORS } from '../../../muiTheme'
import Send from './Send'
import SendPreview from './SendPreview'
import Create from './Create'
import CreatePreview from './CreatePreview'
import { AttestStepId, StepDefinition, StepStatus } from './useAttestSteps'
...
const renderStatusIcon = (status: StepStatus) => {
  ...
  return <RadioButtonUncheckedRounded fontSize="small" style={{ color: COLORS.white }} />
}

const AttestStep = ({ step, derivedActiveStep, canEditStep, onOpenDialog, selectLabel }: AttestStepProps) => {
  ...
  const renderValue = () => {
    ...
    if (shouldShowSelect) {
      return (
        <div className={classes.selectButtonWrapper}>
          <BridgeWidgetButton short className={classes.selectButton} onClick={() => onOpenDialog(step.id)}>
            {selectLabel}
          </BridgeWidgetButton>
        </div>
      )
    ...
    if (isEditable) {
      if (!hasValue) {
        if (step.status === 'pending') {
          return <div className={clsx(classes.stepValue, classes.stepValueStatic, classes.stepValuePlaceholder)}>—</div>
        }
        return <div className={clsx(classes.stepValue, classes.stepValuePlaceholder)}>—</div>
      }
      return (
        <div
          role="button"
    ...
    return (
      <div className={clsx(classes.stepValue, classes.stepValueStatic, !hasValue && classes.stepValuePlaceholder)}>
