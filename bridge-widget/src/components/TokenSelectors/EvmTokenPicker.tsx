...
export default function EvmTokenPicker(props: EthereumSourceTokenSelectorProps) {
  ...
  const getAddress: (address: string, tokenId?: string) => Promise<NFTParsedTokenAccount> = useCallback(
    async (address: string, tokenId?: string) => {
      if (provider && signerAddress && isReady) {
        try {
          const tokenAccount = await (nft ? getEthereumNFT(address, provider) : getEthereumToken(address, provider))
          if (!tokenAccount) {
  ...
  const onChangeWrapper = useCallback(
    async (account: NFTParsedTokenAccount | null) => {
      ...
      if (v1 === true && !migration) {
        throw new Error(t('Wormhole v1 assets cannot be transferred with this bridge.'))
      }
      onChange(account)
      return Promise.resolve()
    },
  ...
  const RenderComp = useCallback(
    ({ account }: { account: NFTParsedTokenAccount }) => {
      return BasicAccountRender(account, isMigrationEligible, nft || false, () => true)
    },
