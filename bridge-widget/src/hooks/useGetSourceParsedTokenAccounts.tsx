...
const createParsedTokenAccountFromInfo = (pubkey: PublicKey, item: AccountInfo<ParsedAccountData>): ParsedTokenAccount => {
  return {
...
const createNativeEthParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
...
const createNativeEthRopstenParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeBscParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativePolygonParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeAvaxParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeOasisParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeAuroraParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeFantomParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeKaruraParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
...
const createNativeAcalaParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
...
const createNativeKlaytnParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
const createNativeCeloParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
...
const createNativeNeonParsedTokenAccount = (provider: Provider, signerAddress: string | undefined) => {
  return !(provider && signerAddress)
    ? Promise.reject()
    : provider.getBalance(signerAddress).then((balanceInWei) => {
        const balanceInEth = ethers.utils.formatEther(balanceInWei)
...
export const getEVMAccounts = async (
  ...
  try {
    ...
    const promises = filteredTokens.map(async (token) => {
      try {
        const tokenId =
          token.tokenChain === sourceChainId
            ? token.nativeAddress
            : await ethers_contracts.BridgeImplementation__factory.connect(
...
const getSolanaParsedTokenAccounts = async (walletAddress: string, dispatch: Dispatch, nft: boolean) => {
...
const getAlgorandParsedTokenAccounts = async (walletAddress: string, dispatch: Dispatch, nft: boolean) => {
  if (nft) {
    // not supported yet
    return
  }
...
function useGetAvailableTokens(nft: boolean = false) {
  ...
  const tokenAccounts = useSelector(nft ? selectNFTSourceParsedTokenAccounts : selectTransferSourceParsedTokenAccounts)
  ...
  const [ethNativeAccount, setEthNativeAccount] = useState<ParsedTokenAccount | undefined>(undefined)
  ...
  const [solanaMintAccounts, setSolanaMintAccounts] = useState<Map<string, ExtractedMintInfo | null> | undefined>(undefined)
  ...
  const resetSourceAccounts = useCallback(() => {
    dispatch(nft ? setSourceWalletAddressNFT(undefined) : setSourceWalletAddress(undefined))
    dispatch(nft ? setSourceParsedTokenAccountNFT(undefined) : setSourceParsedTokenAccount(undefined))
    dispatch(nft ? setSourceParsedTokenAccountsNFT(undefined) : setSourceParsedTokenAccounts(undefined))
    if (!nft) {
      dispatch(setAmount(''))
    }
  ...
  }, [targetChain, selectedTargetChain, selectedSourceWalletAddress, currentSourceWalletAddress, dispatch, resetSourceAccounts])
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_ETH && !ethNativeAccount && !nft) {
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_ETHEREUM_ROPSTEN && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_BSC && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_POLYGON && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_AVAX && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_OASIS && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_AURORA && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_FANTOM && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_KARURA && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_ACALA && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_KLAYTN && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_CELO && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    let cancelled = false
    if (signerAddress && lookupChain === CHAIN_ID_NEON && !ethNativeAccount && !nft) {
      setEthNativeAccountLoading(true)
  ...
  useEffect(() => {
    ...
    if (walletAddress && isEVMChain(lookupChain) && signer !== undefined) {
      if (covalent) {
        dispatch(receiveSourceParsedTokenAccounts(covalent.map((x) => createParsedTokenAccountFromCovalent(walletAddress, x))))
        return
      }
      ...
      getEVMAccounts(lookupChain, targetChain, signer, walletAddress).then(
        (accounts) => {
          if (!cancelled) {
            setCovalentLoading(false)
            setCovalentError(undefined)
            setCovalent(accounts)
            dispatch(receiveSourceParsedTokenAccounts(accounts.map((x) => createParsedTokenAccountFromCovalent(walletAddress, x))))
          }
        },
        () => {
          if (!cancelled) {
            dispatch(
              nft
                ? errorSourceParsedTokenAccountsNFT(t('Cannot load your Ethereum NFTs at the moment.'))
                : errorSourceParsedTokenAccounts(t('Cannot load your Ethereum tokens at the moment.'))
            )
  ...
  const ethAccounts = useMemo(() => {
    ...
    if (ethNativeAccount && output.data) {
      output.data.unshift(ethNativeAccount)
    }
    return output
