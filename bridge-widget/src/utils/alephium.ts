import { ALEPHIUM_REMOTE_TOKEN_POOL_CODE_HASH, getCluster, getConst } from './consts'
import {
  ChainId,
...
async function getTxInfo(provider: NodeProvider, txId: string) {
  ...
  const event = events.events.find((event) => event.contractAddress === getConst('ALEPHIUM_BRIDGE_ADDRESS'))
...
export async function getAlephiumTokenLogoURI(tokenId: string): Promise<string | undefined> {
  return tokenId === ALPH_TOKEN_ID ? alephiumIcon : (await getTokenFromTokenList(tokenId))?.logoURI
}
...
export async function getAndCheckLocalTokenInfo(provider: NodeProvider, tokenId: string): Promise<TokenInfo> {
  ...
  throw new Error(
    i18n.t('Invalid token info, expected: {{ localTokenInfo }}, have: {{ tokenInfo }}', { localTokenInfo: onChainTokenInfo, tokenInfo })
  )
}
...
export async function getAlephiumTokenWrappedInfo(tokenId: string, provider: NodeProvider): Promise<WormholeWrappedInfo> {
  ...
  const tokenAddress = addressFromContractId(tokenId)
  return provider.contracts.getContractsAddressState(tokenAddress).then((state) => {
    if (state.codeHash === ALEPHIUM_REMOTE_TOKEN_POOL_CODE_HASH) {
...
async function fetchRetry<T>(fn: () => Promise<T>, retries: number = 3, delayMs: number = 1000): Promise<T> {
...
async function getAlephiumTokenInfo(tokenId: string, provider: NodeProvider): Promise<(WormholeWrappedInfo & TokenInfo) | undefined> {
  ...
  if ((tokenInfo?.nameOnChain && tokenInfo?.nameOnChain.endsWith(remoteTokenPostfix)) || tokenInfo?.name.endsWith(remoteTokenPostfix)) {
    const tokenAddress = addressFromContractId(tokenId)
    const state = await provider.contracts.getContractsAddressState(tokenAddress)
    if (state.codeHash !== ALEPHIUM_REMOTE_TOKEN_POOL_CODE_HASH) {
...
export async function createLocalTokenPool(
  ...
  const result = await createLocalTokenPoolOnAlph(
    signer,
    getAttestTokenHandlerId(getConst('ALEPHIUM_TOKEN_BRIDGE_CONTRACT_ID'), CHAIN_ID_ALEPHIUM, getConst('ALEPHIUM_BRIDGE_GROUP_INDEX')),
    localTokenId,
    signedVaa,
    payer,
    MINIMAL_CONTRACT_DEPOSIT
...
export async function getIsTxsCompletedAlph(tokenBridgeForChainId: string, sequences: bigint[]): Promise<boolean[]> {
  ...
  for (const seq of sequences) {
    if (contractState.fields.start > seq) {
      results.push(await isSequenceExecuted(tokenBridgeForChainId, seq, getConst('ALEPHIUM_BRIDGE_GROUP_INDEX')))
      continue
    }
