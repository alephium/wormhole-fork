...
const MAINNET_CHAINS_WITH_NFT_SUPPORT: ChainInfo[] = MAINNET_CHAINS.filter(({ id }) => id === CHAIN_ID_ETH)
const TESTNET_CHAINS_WITH_NFT_SUPPORT: ChainInfo[] = TESTNET_CHAINS.filter(({ id }) => id === CHAIN_ID_ETH)
...
TESTNET_BSC_MARKET_WARNINGS[0] = getAddress(bscTestnetConfig.contracts.wrappedNative)

const DEVNET_BSC_MARKET_WARNINGS = [...MAINNET_BSC_MARKET_WARNINGS]
DEVNET_BSC_MARKET_WARNINGS[0] = getAddress(bscDevnetConfig.contracts.wrappedNative)
...
export const getConst = (
  ...
  switch (constant) {
    case 'CHAINS':
      return CLUSTER === 'mainnet' ? MAINNET_CHAINS : CLUSTER === 'testnet' ? TESTNET_CHAINS : MAINNET_CHAINS
    ...
    case 'CHAINS_BY_ID':
      return CLUSTER === 'mainnet' ? MAINNET_CHAINS_BY_ID : CLUSTER === 'testnet' ? TESTNET_CHAINS_BY_ID : MAINNET_CHAINS_BY_ID
    ...
    case 'ETH_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 1 : CLUSTER === 'testnet' ? 11155111 : 1338
    ...
    case 'AVAX_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 43114 : CLUSTER === 'testnet' ? 43113 : 1381

    case 'OASIS_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 42262 : CLUSTER === 'testnet' ? 42261 : 1381

    case 'AURORA_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 1313161554 : CLUSTER === 'testnet' ? 1313161555 : 1381
    ...
    case 'CELO_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 42220 : CLUSTER === 'testnet' ? 44787 : 1381

    case 'NEON_NETWORK_CHAIN_ID':
      return CLUSTER === 'mainnet' ? 245022934 : CLUSTER === 'testnet' ? 245022926 : 1381
    ...
    case 'ALGORAND_HOST':
      return CLUSTER === 'mainnet' ? ALGORAND_MAINNET_HOST : CLUSTER === 'testnet' ? ALGORAND_TESTNET_HOST : ALGORAND_DEVNET_HOST
    ...
    case 'ALGORAND_BRIDGE_ID':
      return BigInt(CLUSTER === 'mainnet' ? '0' : CLUSTER === 'testnet' ? '86525623' : '4')
    case 'ALGORAND_TOKEN_BRIDGE_ID':
      return BigInt(CLUSTER === 'mainnet' ? '0' : CLUSTER === 'testnet' ? '86525641' : '6')
    ...
    case 'ALEPHIUM_POLLING_INTERVAL':
      return CLUSTER === 'mainnet' ? 10000 : CLUSTER === 'testnet' ? 10000 : 1000
    ...
    case 'ETH_MIGRATION_ASSET_MAP':
      return CLUSTER === 'mainnet' ? MAINNET_ETH_MIGRATION_ASSET_MAP : TESTNET_ETH_MIGRATION_ASSET_MAP
    ...
    case 'MULTI_CHAIN_TOKENS':
      ...
      return CLUSTER === 'mainnet' ? MAINNET_MULTI_CHAIN_TOKENS : TESTNET_MULTI_CHAIN_TOKENS

    case 'RELAYER_INFO_URL':
      return CLUSTER === 'mainnet' ? '' : CLUSTER === 'testnet' ? '' : '/relayerExample.json'
    // also for karura
...
export const COVALENT_GET_TOKENS_URL = (chainId: ChainId, walletAddress: string, nft?: boolean, noNftMetadata?: boolean) => {
  if (COVALENT_API_KEY === '') {
    return ''
  }
  const chainNum =
...
export const TVL_CUMULATIVE_URL = 'https://europe-west3-wormhole-315720.cloudfunctions.net/mainnet-notionaltvlcumulative?totalsOnly=true'
...
export const NOTIONAL_TRANSFERRED_URL = 'https://europe-west3-wormhole-315720.cloudfunctions.net/mainnet-notionaltransferredfrom'
...
export const AVAILABLE_MARKETS_URL = 'https://docs.wormholenetwork.com/wormhole/overview-liquid-markets'
...
export const logoOverrides = new Map<string, string>([
  ['0x727f064a78dc734d33eec18d5370aef32ffd46e4', 'https://orion.money/assets/ORION-LOGO-2.1-GREEN@256x256.png']
])
...
export const getIsTransferDisabled = (chainId: ChainId, isSourceChain: boolean) => {
  const disableTransfers = CHAIN_CONFIG_MAP[chainId]?.disableTransfers
  return disableTransfers === 'from' ? isSourceChain : disableTransfers === 'to' ? !isSourceChain : !!disableTransfers
}
...
export const getCoinGeckoURL = (coinGeckoId: string) => `https://api.coingecko.com/api/v3/simple/price?ids=${coinGeckoId}&vs_currencies=usd`

export const RELAY_URL_EXTENSION = '/relayvaa/'

export const getChainShortName = (chainId: ChainId) => {
  return chainId === CHAIN_ID_BSC ? 'BSC' : getConst('CHAINS_BY_ID')[chainId]?.name
}
...
export const getIsTokenTransferDisabled = (chainId: ChainId, tokenAddress: string) => {
