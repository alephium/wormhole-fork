...
export class VaaBundleFetcher {
  ...
  private idToKey = (id: VaaId) =>
    `${id.emitterChain}/${uint8ArrayToHex(id.emitterAddress)}/${id.targetChain}/${id.sequence.toString()}`;
  ...
  addVaaPayload(parsedVaa: ParsedVaaWithBytes) {
    const id: VaaId = {
      emitterChain: parsedVaa.parsed.body.emitterChainId,
      emitterAddress: parsedVaa.parsed.body.emitterAddress,
      targetChain: parsedVaa.parsed.body.targetChainId,
      sequence: parsedVaa.parsed.body.sequence
    }
  ...
  serialize(): SerializedBatchFetcher {
    return {
      vaaBytes: Object.values(this.fetchedVaas).map(parsedVaas =>
        Buffer.from(parsedVaas.bytes).toString("base64"),
      ),
