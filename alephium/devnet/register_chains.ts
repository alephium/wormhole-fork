...
export async function registerChains(wormhole: Wormhole, tokenBridgeId: string): Promise<RemoteChains> {
    const vaas = [
        ...
        // Terra, sequence = 1
        '',
        // BSC, sequence = 3
        ''
    ]

    var txId = await wormhole.registerChainToAlph(tokenBridgeId, vaas[0], env.payer, env.dustAmount)
    ...
    txId = await wormhole.registerChainToAlph(tokenBridgeId, vaas[1], env.payer, env.dustAmount)
    ...
    txId = await wormhole.registerChainToAlph(tokenBridgeId, vaas[2], env.payer, env.dustAmount)
