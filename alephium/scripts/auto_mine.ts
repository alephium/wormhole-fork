import { addressFromPublicKey, NodeProvider } from "@alephium/web3"
...
const nodeProvider = new NodeProvider(configuration.networks["devnet"].nodeUrl)

async function mine(provider: NodeProvider): Promise<void> {
  const signer = new PrivateKeySigner(provider, fromPrivateKey)
