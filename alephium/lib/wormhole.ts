...
export class Wormhole {
    ...
    private async getDevnetDeployerId(): Promise<string> {
        if (typeof this._devnetDeployerId === 'undefined') {
            const devnetDeployerContract = await Contract.fromSource(this.provider, 'devnet_deployer.ral')
    ...
    private async deployGovernance(
        ...
        if (devnet) {
            ...
            const script = await Script.fromSource(this.provider, 'deploy_governance.ral')
    ...
    private async deployTokenBridge(
        ...
        if (devnet) {
            const deployerId = await this.getDevnetDeployerId()
            const script = await Script.fromSource(this.provider, 'deploy_token_bridge.ral')
            const scriptTx = await script.transactionForDeployment(this.signer, {
                initialFields: {
                    'deployerId': deployerId,
