...
export class WormholeInstructionCoder implements InstructionCoder {
  ...
  public decode(
    ...
    let ixBytes = typeof ix === "string"
      ? encoding === "hex" ? Buffer.from(ix, "hex") : Buffer.from(bs58.decode(ix))
      : ix
    let discriminator = ixBytes.slice(0, 1).readInt8();
    let data = ixBytes.slice(1);
