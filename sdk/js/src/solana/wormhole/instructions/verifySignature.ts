...
export async function createVerifySignaturesInstructions(
  ...
  const parsed = deserializeVAA(vaa);
  ...
  const guardianSignatures = parsed.signatures;
  ...
  for (let i = 0; i < Math.ceil(guardianSignatures.length / batchSize); ++i) {
    ...
    for (let j = 0; j < end - start; ++j) {
      ...
      signatures.push(Buffer.from(item.sig));
    ...
    instructions.push(
      createSecp256k1Instruction(signatures, keys, Buffer.from(vaaHash))
    );
    instructions.push(
